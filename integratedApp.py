...
def format_timestamp(timestamp_str: str, format_type: str = "relative") -> str:
    """Format timestamp string for display"""
    try:
        if timestamp_str.endswith('Z'):
            timestamp_str = timestamp_str[:-1] + '+00:00'
        
        dt = datetime.fromisoformat(timestamp_str).replace(tzinfo=None)
        now = datetime.now()
...
def extract_keywords_from_query(query: str):
    ...
    stop_words = {
        ...
        'now', 'show', 'me', 'find', 'get', 'give', 'tell', 'see', 'look',
        'what', 'where', 'when', 'how', 'why', 'who', 'which', 'that', 'this'
    }
...
def generate_enhanced_logs(count: int = 500):
    """Generate enhanced log data with metadata and transaction tracing"""
    services = [
        "payment-api", "user-service", "auth-service", "order-processor", 
        "notification-service", "inventory-api", "shipping-service", "analytics-engine",
        "operation-user-interface-app", "document-notification-api"
    ]
    ...
    error_messages = [
        ...
        "Invalid input parameter: user_id",
        "Service unavailable: maintenance mode",
        "Memory allocation failed",
        "Network unreachable: host timeout",
    ...
    warning_messages = [
        ...
        "Slow query execution time: 2.5s",
        "Retry attempt 3/5 for failed operation",
        "API response time exceeding threshold",
        "Cache miss rate above normal: 45%",
        "Connection pool usage high: 90%",
        "Disk space usage warning: 80%",
        "Background job queue backlog detected"
    ]
    ...
    for i in range(count):
        ...
        if level == "ERROR":
            log_entry["metadata"].update({
                "error_code": random.choice([500, 502, 503, 504, 400, 401, 403, 404]),
                "stack_trace": "com.example.service.Exception at line 123"
            })
        
        if random.random() < 0.3:
            log_entry["metadata"]["duration_ms"] = random.randint(10, 5000)
...
def generate_sample_alerts():
    ...
    services = [
        "payment-api", "user-service", "auth-service", "order-processor",
        "notification-service", "inventory-api", "shipping-service"
    ]
    ...
    for i in range(random.randint(3, 8)):
        alert_time = datetime.now() - timedelta(minutes=random.randint(1, 120))
        ...
        alert = {
            ...
            "message": f"Alert detected in {random.choice(services)}",
            "status": random.choice(["ACTIVE", "RESOLVED", "ACKNOWLEDGED"]),
...
def generate_transaction_trace():
    """Generate a sample transaction trace for root cause analysis"""
    trace_id = str(uuid.uuid4())
    ...
    return {
        ...
        "error_message": "Payment processing failed due to gateway timeout"
    }

def generate_ai_story(timeframe, logs_data, metrics_data):
    """Generate AI story summary based on system data"""
    total_logs = len(logs_data)
    error_logs = len([log for log in logs_data if log.get('level') == 'ERROR'])
    error_rate = (error_logs / total_logs * 100) if total_logs > 0 else 0
    
    services = list(set([log.get('service', 'unknown') for log in logs_data]))
    
    if metrics_data is not None:
        avg_cpu = metrics_data['cpu_usage'].mean()
        avg_response = metrics_data['response_time'].mean()
        max_response = metrics_data['response_time'].max()
    else:
        avg_cpu = 45
        avg_response = 250
        max_response = 500
    
    # Generate narrative based on data patterns
    stories = []
    
    if error_rate > 10:
        stories.append(f"The system experienced significant instability with {error_rate:.1f}% error rate. Critical issues were detected across {len(services)} services.")
    elif error_rate > 5:
        stories.append(f"Moderate service disruptions occurred with {error_rate:.1f}% error rate. Several services showed degraded performance.")
    else:
        stories.append(f"The system operated smoothly with only {error_rate:.1f}% error rate, indicating healthy service performance.")
    
    if avg_response > 1000:
        stories.append(f"Response times were critically high, averaging {avg_response:.0f}ms with peaks reaching {max_response:.0f}ms.")
    elif avg_response > 500:
        stories.append(f"Response times showed degradation, averaging {avg_response:.0f}ms, suggesting potential bottlenecks.")
    else:
        stories.append(f"Response times remained healthy at {avg_response:.0f}ms average, ensuring good user experience.")
    
    if avg_cpu > 80:
        stories.append(f"CPU utilization was critically high at {avg_cpu:.1f}%, indicating resource constraints.")
    elif avg_cpu > 60:
        stories.append(f"CPU usage was elevated at {avg_cpu:.1f}%, suggesting increased system load.")
    else:
        stories.append(f"CPU utilization remained optimal at {avg_cpu:.1f}%, showing efficient resource management.")
...
with st.spinner('Loading dashboard data...'):
    ...
    enhanced_logs = generate_enhanced_logs(500)
    ...
    transaction_trace = generate_transaction_trace()
...
enhanced_logs_df = pd.DataFrame(enhanced_logs)
...
st.markdown("## 🧠 AI System Story")
col1, col2 = st.columns([3, 1])

with col1:
    st.markdown("Get AI-generated insights about your system's behavior and performance patterns.")

with col2:
    if st.button("📚 Generate Story", type="primary"):
        with st.spinner("Analyzing system data and generating insights..."):
            # Generate AI story using enhanced logs and metrics
            story_result = generate_ai_story("24h", enhanced_logs, time_series_data)
            ...
            if story_result['status'] == 'success':
                ...
                ">
                    <h3 style="margin-top: 0; color: white;">System Narrative (Last 24 Hours)</h3>
                    <div style="font-size: 1.1em; line-height: 1.6; font-style: italic;">
                        "{story_result['story']}"
                    </div>
                    <div style="margin-top: 1rem; font-size: 0.9em; opacity: 0.8;">
                        Based on {story_result['log_count']} logs and {len(story_result['key_metrics'])} key metrics
                    </div>
                </div>
                """, unsafe_allow_html=True)
                ...
                with col1:
                    st.metric("Error Rate", f"{story_result['key_metrics']['error_rate']:.1f}%")
                with col2:
                    st.metric("Avg Response Time", f"{story_result['key_metrics']['avg_response_time']:.0f}ms")
                with col3:
                    st.metric("Avg CPU Usage", f"{story_result['key_metrics']['avg_cpu']:.1f}%")
                with col4:
                    st.metric("Active Services", story_result['key_metrics']['services_count'])
...
with st.sidebar:
    ...
    show_logs = st.checkbox("📋 Enhanced Logs", value=True)
    show_alerts = st.checkbox("🚨 Alert Analysis", value=True)
    show_tracing = st.checkbox("🔍 Transaction Tracing", value=True)
    show_activity = st.checkbox("🔥 Activity Timeline", value=True)
    
    st.markdown("---")
    
    # Advanced filtering
    st.markdown("### 🔍 Advanced Filters")
    log_level_filter = st.selectbox("Log Level", ["All", "ERROR", "WARN", "INFO", "DEBUG"])
    service_filter = st.multiselect("Services", options=list(set([log.get('service', 'unknown') for log in enhanced_logs])))
    
    # Natural language query
    st.markdown("### 💬 Natural Language Query")
    nl_query = st.text_input("Ask about your system", placeholder="Show me errors in payment service")
    if nl_query:
...
if show_alerts:
    ...
    with col2:
        ...
        fig_alert_pie.update_layout(height=300)
        st.plotly_chart(fig_alert_pie, use_container_width=True)

# Transaction Tracing
if show_tracing:
    st.markdown("## 🔍 Transaction Tracing")
    
    col1, col2 = st.columns([3, 1])
    
    with col1:
        st.markdown("### Sample Transaction Trace")
        
        # Create Gantt chart for transaction trace
        trace_df = pd.DataFrame(transaction_trace['calls'])
        
        fig_trace = px.timeline(
            trace_df,
            x_start='start_time',
            x_end=[call['start_time'] + call['duration'] for call in transaction_trace['calls']],
            y='service',
            color='status',
            title=f"Transaction Trace: {transaction_trace['trace_id'][:8]}...",
            color_discrete_map={'SUCCESS': '#2ca02c', 'ERROR': '#d62728'}
        )
        
        fig_trace.update_layout(height=300)
        st.plotly_chart(fig_trace, use_container_width=True)
    
    with col2:
        st.markdown("### Trace Summary")
        st.metric("Total Duration", f"{transaction_trace['total_duration']}ms")
        st.metric("Status", transaction_trace['status'])
        st.metric("Root Cause Service", transaction_trace['root_cause_service'])
        
        if transaction_trace['status'] == 'ERROR':
            st.error(f"Error: {transaction_trace['error_message']}")

# Activity Timeline
if show_activity:
    st.markdown("## 🔥 Activity Timeline")
    
    # Enhanced logs activity timeline
    st.markdown("### Log Activity Over Time")
    
    enhanced_logs_df_copy = enhanced_logs_df.copy()
    enhanced_logs_df_copy['timestamp'] = pd.to_datetime(enhanced_logs_df_copy['timestamp'])
    enhanced_logs_df_copy['time_bucket'] = enhanced_logs_df_copy['timestamp'].dt.floor('10T')
    
    timeline_data = enhanced_logs_df_copy.groupby(['time_bucket', 'level']).size().reset_index(name='count')
    ...
    fig_timeline = px.bar(
        ...
        y='count',
        color='level',
        title="Enhanced Log Activity Timeline (10-minute intervals)",
        color_discrete_map={
            'ERROR': '#ff4444',
            'WARN': '#ffaa00',
            'INFO': '#44aa44',
    ...
    service_time_data = enhanced_logs_df_copy.groupby([
        enhanced_logs_df_copy['timestamp'].dt.floor('h'), 
        'service'
